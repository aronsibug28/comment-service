...
class Contentservice {
  ...
  registerLayoutType(
    ...
    const propertyComponentName = `${componentName}-props`
  ...
  toolboxCategories() {
    ...
    for (var name in this.knownElementTypes) {
      if (this.knownElementTypes.hasOwnProperty(name)) {
        const type = this.knownElementTypes[name]
    ...
    categories.forEach((category) => {
      ...
      for (var name in category.types) {
        if (category.types.hasOwnProperty(name)) {
          const type = category.types[name]
          arr.push(type)
        }
      }
      category.types = arr
  ...
  toggleEditMode() {
    ...
    const previousEditMode = this.store.state.previousEditMode
    if (mode === 'view') {
  ...
  select(vm, param1, param2) {
    return new Promise((resolve, reject) => {
      ...
      if (arguments.length === 3) {
        ...
        reject(new Error('Unknown parameters to CrowdHound.select'))
        // if (type1 == 'function') {
        //   var callback = param1;
